...
export const register = async (req, res) => {
    ...
    if (picturePath.length > 0) {
        const data = await cloudinary.v2.uploader.upload(picturePath[0], {
            folder: "TwikTik",
        });
    ...
    db.query(q, [email], async (err, data) => {
        if (err) {
            ...
            q =
                "INSERT INTO users (`firstName`,`lastName`,`location`,`occupation`,`email`,`password`,`picturePath`,`public_url`) VALUES (?,?,?,?,?,?,?,?)";
    ...
    db.query(q, [email], (err, data) => {
        if (err) return res.status(500).json({ success: false, err });
            ...
            if (data.length) {
                ...
                } else return sendToken(data, 200, res);
            } else
                return res
                    .status(404)
