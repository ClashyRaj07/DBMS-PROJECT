...
export const createPost= async (req, res) => {
  ...
  const token=req.cookies.token;
    ...
    jwt.verify(token,"secretKey",(err,userInfo)=>{
        ...
        let q = "INSERT INTO posts (`description`,`post_img`,`userId`,`public_url`) VALUES (?,?,?,?)"
        const values=[description,req.body.post_img,userInfo.id,req.body.public_url]

        db.query(q, values , (err, data) => {
            if (err) return res.status(500).json({success: false,data:err})
            return res.status(200).json({success:true,data:"Post has been Creaed."})  
        })
...
// Get Post by postId
export const getPost=('/post/:postId', (req, res) => {
